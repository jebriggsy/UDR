...
void usage(bool do_exit) {
  ...
  fprintf(
      stderr,
      "\t[-n aes-128 | aes-192 | aes-256 | bf | des-ede3] Encryption cypher\n");
...
ostream &UDR_Options::err() { return *mycerr << which_process << "(error) "; }

ostream &UDR_Options::err(int errnum) {
  return *mycerr << which_process << "(error " << errnum << ":"
                 << strerror(errnum) << ") ";
}
ostream &UDR_Options::err(UDT::ERRORINFO &err) {
  return *mycerr << which_process << "(UDT error " << err.getErrorCode() << ":"
                 << err.getErrorMessage() << ") ";
}
...
int UDR_Options::get_options(int argc, char *argv[]) {
  ...
  // parse opptions, stop at the first non-option (for ssh compatibility when
  // invoked by rsync) other options needed for ssh compatibility: l
  // (login-name)
  const char *opts = "+P:i:tl:vxa:b:s:d:h:p:c:k:o:r:n::";

  int ch;
  while ((ch = getopt_long(argc, argv, opts, long_options, &option_index)) !=
         -1) {
    switch (ch) {
    case 'P':
  ...
  if (timeout < 1) {
    cerr << "Please specify a timeout duration [-d timeout] greater than 0s."
         << endl;
    exit(1);
  }
  ...
  if (sflag) {
    ...
    for (size_t i = 1; i < extra_args.size(); i++) {
      ...
      if (arg.find("-e") == 0 || arg == "--rsh") {
        cerr << "UDR ERROR: UDR overrides the -e, --rsh flag of rsync, so they "
                "cannot be used in the provided rsync command"
             << endl;
        throw udr_exitexception(1);
      }
    }
  ...
  if (is_verbose()) {
    ...
    // for(udr_args::iterator it=extra_args.begin(); it != extra_args.end();
    // it++)
    //     cerr << " \"" << *it << '"';
    cerr << endl;
  }
...
void parse_host_username(const std::string &source, std::string &username,
                         std::string &hostname, bool &double_colon) {
...
void UDR_Options::get_host_username() {
  ...
  for (ssize_t i = extra_args.size() - 2; i > 0; i--) {
    ...
    parse_host_username(extra_args[i], next_src_username, next_src_host,
                        next_src_double_colon);
    ...
    if (src_num == 0) {
      ...
      // if we have a hostname, we must ensure that it is the same as the
      // sources if we don't have a hostname, this could be an option argument
      // and not a source
      if (src_username != next_src_username || src_host != next_src_host ||
          src_double_colon != next_src_double_colon) {
        // have a problem
        cerr << "UDR ERROR: sources must use the same host and username"
             << endl;
        exit(-1);
      }
    }
  ...
  if (src_remote == dest_remote) {
    cerr << "UDR ERROR: UDR only does remote -> local or local -> remote "
            "transfers"
         << endl;
    exit(-1);
  }
