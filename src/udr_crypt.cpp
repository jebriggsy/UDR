...
udr_crypt::udr_crypt(crypt_dir_t direc, const std::string &encryption_type,
                     const key_t &key)
    : direction(direc) {
...
bool udr_crypt::init(const std::string &encryption_type, const key_t &key) {
  ...
  if (key.size() < rlen) {
    goptions.err() << "key short " << key.size() << "bytes, " << rlen
                   << " required" << endl;
    return false;
  }
  ...
  if (!ctxp ||
      EVP_CipherInit_ex(ctxp, cipher, NULL, key.data(), ivec, direction)) {
...
udr_crypt::cipher_t udr_crypt::get_cipher(const std::string &encryption_type) {
  ...
  if (encryption_type == "aes-128") {
    ...
    goptions.err() << "error unsupported encryption type: " << encryption_type
                   << std::endl;
    exit(EXIT_FAILURE);
  }
...
int udr_crypt::encrypt(char *in, char *out, int len) {
  ...
  if (!EVP_CipherUpdate(ctxp, (unsigned char *)out, &evp_outlen,
...
std::string udr_crypt::encode_64(const key_t &bytes) {
  ...
  int n = EVP_EncodeBlock((unsigned char *)result.data(), bytes.data(),
                          bytes.size());
  result.resize(n);
  return result;
}
...
udr_crypt::key_t udr_crypt::decode_64(const std::string &text) {
  ...
  int n = EVP_DecodeBlock(result.data(), (const unsigned char *)text.data(),
                          text.size());
  if (n != -1)
    result.resize(n);
  else
