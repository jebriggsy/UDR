...
udr_process::udr_process(const std::vector<std::string> &args, bool capture,
                         bool tty) {
  init();
  if (tty) {
...
bool udr_process::waitable() const noexcept { return pid != 0; }
...
bool udr_process::wait(int timeout_ms) {
  ...
  bool ok = false;
  {
    ...
    if (timeout_ms == 0) {
      ...
      ok = cv.wait_for(lock, std::chrono::milliseconds(timeout_ms),
                       [&] { return handled.count(pid) > 0; });
    }
    if (ok)
      handled.erase(pid);
  }
...
pid_t udr_process::get_id() const noexcept { return pid; }
...
pid_t fork_execvp(const std::string &what, const udr_args &cmdargs, int *ptc,
                  int *ctp) {
  if (!cmdargs.size()) {
...
pid_t fork_execvp_pty(const std::string &what, const udr_args &cmdargs,
                      int &master) {
  if (!cmdargs.size()) {
    throw udr_argexception(std::string("mising command for ") + what);
  }
