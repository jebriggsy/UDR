...
thread_data::thread_data()
    : udt_socket(NULL), fd(0), id(-1), crypt(NULL), debug(false), log(false),
      is_complete(false) {}
...
void *monitor_timeout(void *_arg) {
  ...
  while (1) {
    ...
    // If timeout_sem == 2, the connection has not been made -> no timeout next
    // round
    if (timeout_sem != 2)
      timeout_sem = 0;
  }
...
string udt_recv_string(int udt_handle) {
  ...
  for (;;) {
    ...
    if (bytes_read == UDT::ERROR) {
      goptions.err() << "recv:" << UDT::getlasterror().getErrorMessage()
                     << endl;
      break;
    }
    if (bytes_read == 1) {
...
void sigexit(int signum) { exit(EXIT_SUCCESS); } /* Exit successfully */
...
void *handle_to_udt(void *threadarg) {
  ...
  if (my_args->log) {
    string filename =
        my_args->logfile_dir + convert_int(my_args->id) + "_log.txt";
...
void *udt_to_handle(void *threadarg) {
  ...
  while (true) {
    ...
    if (UDT::ERROR ==
        (rs = UDT::recv(*my_args->udt_socket, indata, max_block_size, 0))) {
      if (my_args->log) {
...
int run_sender(const UDR_Options &udr_options, const std::string &passphrase,
  ...
  goptions.verb() << " connecting to " << udr_options.host << ":"
                  << udr_options.port_num << endl;
  if (0 !=
      getaddrinfo(udr_options.host.c_str(), port_num.c_str(), &hints, &peer)) {
    goptions.err() << " incorrect server/peer address. " << udr_options.host
                   << ":" << udr_options.port_num << endl;
    return 1;
  }
  ...
  if (UDT::ERROR == UDT::connect(client, peer->ai_addr, peer->ai_addrlen)) {
    goptions.err() << " connect: " << UDT::getlasterror().getErrorMessage()
                   << endl;
    return 1;
  }
  ...
  // very first thing we send is the rsync argument so that the rsync server can
  // be started and piped to from the UDT connection
  n = cmd.size() + 1;
  int ssize = 0;
  while (ssize < n) {
    int ss;
    if (UDT::ERROR ==
        (ss = UDT::send(client, cmd.c_str() + ssize, n - ssize, 0))) {
      goptions.err() << " Send:" << UDT::getlasterror().getErrorMessage()
                     << endl;
      break;
    }
  ...
  sender_to_udt.fd = STDIN_FILENO; // stdin of this process, from stdout of
                                   // rsync
  ...
  udt_to_sender.fd =
      STDOUT_FILENO; // stdout of this process, going to stdin of rsync, rsync
                     // defaults to set this is non-blocking
  udt_to_sender.log = thread_log;
  udt_to_sender.logfile_dir = local_logfile_dir;

  if (udr_options.encryption) {
    udr_crypt encrypt(udr_crypt::ENCRYPT, udr_options.encryption_type,
                      passphrase);
    udr_crypt decrypt(udr_crypt::DECRYPT, udr_options.encryption_type,
                      passphrase);
    // free_key(passphrase);
  ...
  pthread_create(&sender_to_udt_thread, NULL, handle_to_udt,
                 (void *)&sender_to_udt);

  pthread_t udt_to_sender_thread;
  pthread_create(&udt_to_sender_thread, NULL, udt_to_handle,
                 (void *)&udt_to_sender);
...
int run_receiver(const UDR_Options &udr_options) {
  ...
  if (specify_ip)
    goptions.verb() << "Specifying on specific ip: " << udr_options.specify_ip
                    << endl;
  ...
  if (udr_options.start_port > udr_options.end_port) {
    goptions.err() << " ERROR: invalid port range: " << udr_options.start_port
                   << " - " << udr_options.end_port << endl;
    ;
    return 0;
  }

  for (int port_num = udr_options.start_port; port_num <= udr_options.end_port;
       port_num++) {
    bad_port = false;
    ...
    if (0 != getaddrinfo(NULL, receiver_port.c_str(), &hints, &res)) {
      ...
      if (specify_ip) {
        ...
        r = UDT::bind(serv, (struct sockaddr *)&my_addr,
  ...
  if (bad_port) {
    goptions.err() << " ERROR: could not bind to any port in range: "
  ...
  goptions.verb() << " server is ready at port " << receiver_port << " pw "
                  << ekey << endl;

  if (UDT::ERROR == UDT::listen(serv, 10)) {
    goptions.err() << " listen: " << UDT::getlasterror().getErrorMessage()
                   << endl;
    return 0;
  }
  ...
  if (UDT::INVALID_SOCK ==
    ...
    goptions.err() << " accept: " << UDT::getlasterror().getErrorMessage()
                   << endl;
    return 0;
  }
  ...
  // If in server mode, need to check that --sender is a option (read-only) and
  // change the directory to be in the directory that is being served up.
  ...
  if (udr_options.server_connect) {
    ...
    if (!udr_options.server_config.empty()) {
      sprintf(rsync_cmd, "%s%s %s",
              "rsync --config=", udr_options.server_config.c_str(),
              " --server --daemon .");
    } else {
  ...
  // now fork and exec the rsync on the remote side using sh (so that wildcards
  // will be expanded properly)
  int child_to_parent, parent_to_child;
  int rsync_pid = fork_execvp((char *)udr_options.shell_program.c_str(), sh_cmd,
  ...
  pthread_create(&recv_to_udt_thread, NULL, handle_to_udt,
                 (void *)&recv_to_udt);

  pthread_t udt_to_recv_thread;
  pthread_create(&udt_to_recv_thread, NULL, udt_to_handle,
                 (void *)&udt_to_recv);
  ...
  // going to poll if the ppid changes then we know it's exited and then we exit
  // all of our threads and exit as well also going to check if either thread is
  // complete, if one is then the other should also be killed bit of a hack to
  // deal with the pthreads
  while (true) {
    ...
    if (recv_to_udt.is_complete && udt_to_recv.is_complete) {
      if (udr_options.verbose) {
        fprintf(stderr,
                "[udr receiver] both threads are complete: "
                "recv_to_udt.is_complete %d udt_to_recv.is_complete %d\n",
      ...
      if (udr_options.verbose) {
        fprintf(stderr,
                "[udr receiver] recv_to_udt is complete: "
                "recv_to_udt.is_complete %d udt_to_recv.is_complete %d\n",
                recv_to_udt.is_complete, udt_to_recv.is_complete);
      }
      break;
      ...
      if (udr_options.verbose) {
        fprintf(stderr,
                "[udr receiver] udt_to_recv is complete: "
                "recv_to_udt.is_complete %d udt_to_recv.is_complete %d\n",
                recv_to_udt.is_complete, udt_to_recv.is_complete);
      }
      break;
  ...
  // if(udr_options.verbose){
  // fprintf(stderr, "[udr receiver] Joined udt_to_recv_thread %d Should be
  // closing recver now\n", rc2);
  // }
...
bool udr_thread::start() {
  int res =
      pthread_create(&thread, NULL, _thread_func, static_cast<void *>(this));
  if (res) {
