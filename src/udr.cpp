...
int main(int argc, char *argv[]) {
  ...
  //
  // TODO: Get someone who knows C to verify this can be removed
  // Note from jebriggsy merge
  //
...
// only going to go from local -> remote and remote -> local, remote <-> remote
// maybe later, but local -> local doesn't make sense for UDR
...
int run_udr_main(UDR_Options &options) {
   ...
   * options.server_connect, options.server is for the udr server */
  if (options.server_connect) {
    options.verb() << " trying server connection" << endl;

    int server_exists = get_server_connection(options.host, options.server_port,
                                              udr_cmd.c_str(), line, line_size);

    if (!server_exists) {
      cerr << "UDR ERROR: Cannot connect to server at " << options.host << ":"
           << options.server_port << endl;
      exit(EXIT_FAILURE);
    }
  }
  /* If not try ssh */
  else {
    // We are starting the ssh child process to start udr executable on the
    // other side!
    ...
    if (nbytes <= 0) {
      options.err() << "UDR ERROR: unexpected response from remote, exiting."
                    << endl;
      exit(EXIT_FAILURE);
    }
  }
  ...
  options.verb() << " port_num: " << options.port_num
                 << " passphrase: " << hex_pp << endl;
  ...
  if (options.encryption) {
    ...
    if (key_file == NULL || fail) {
      options.err() << "UDR ERROR: could not write key file: "
                    << options.key_filename << endl;
      exit(EXIT_FAILURE);
    }
...
int run_udr_rsh_client(UDR_Options &options) {
  ...
  if (options.encryption) {
    ...
    if (key_file == NULL) {
      cerr << options.which_process
           << " UDR ERROR: could not read from key_file "
           << options.key_filename << endl;
      exit(EXIT_FAILURE);
    }
  ...
  options.verb() << " rsh host: " << options.host << " cmd: \""
                 << remote_command << "\"" << endl;
...
// get the command to invoke a rsh like version of udr, for rsync to communicate
// over
...
void print_version() { cerr << "UDR version " << version << endl; }
