...
void udr_rsh_base::close(bool abortive) {
  ...
  if (s) {
    goptions.dbg2() << "closing udt sock " << s
                    << (abortive ? " (abortive)" : "") << std::endl;
    if (abortive) {
...
int udr_rsh_base::get_child_status() const { return child.exit_status(); }
...
void udr_rsh_base::start_child(const std::string &purpose,
...
bool udr_rsh_base::udt_send_string(const std::string &str) {
  ...
  while (total_sent < total_size) {
    int bytes_sent =
        UDT::send(socket, str.c_str() + total_sent, total_size - total_sent, 0);
...
// receive a null terminated string, which is ow rsh_local sends the command to
// run
...
bool udr_rsh_remote::run() {
  ...
  if (!start_pump(socket, from_child, to_child)) {
    // child process isnt explicitly killed, it will die once it doesnt receive
    // input/output
    return false;
  }
  ...
  bool abortive = false;
  {
    ...
    while (!pump->should_stop()) {
      ...
      if (stdin_closed) {
        // just leave things and exit.  There is no one to report exit status
        // to, or anything.
...
bool udr_rsh_remote::bind_server() {
  ...
  if (specify_ip)
    goptions.verb() << "Specifying on specific ip: " << goptions.specify_ip
                    << endl;
  ...
  int port_num;
  for (port_num = goptions.start_port; port_num <= goptions.end_port;
       port_num++) {
    bad_port = false;
  ...
  if (bad_port) {
    goptions.err() << "ERROR: could not bind to any port in range: "
                   << goptions.start_port << " - " << goptions.end_port << endl;
    ;
    return false;
  }
...
bool udr_rsh_remote::accept(int ms) {
  ...
  while (ms <= 0 || remaining) {
    ...
    if (r == 0) {
      ...
      if (UDT::INVALID_SOCK ==
          (socket = UDT::accept(serv, (sockaddr *)&clientaddr, &addrlen))) {
...
std::string udr_rsh_remote::get_command(const std::string &cmd) {
  if (goptions.server_connect) {
    ...
    if (!goptions.server_config.empty()) {
      return std::string("rsync --config=") + goptions.server_config +
             "--server --daemon .";
    } else {
...
bool udr_rsh_local::run(const std::string &host, int port,
                        const std::string &cmd) {
...
bool udr_rsh_local::connect(const std::string &host, int port, int ms) {
  ...
  if (0 != getaddrinfo(host.c_str(), s_port.c_str(), &hints, &peer)) {
    goptions.err() << " incorrect server/peer address. " << host << ":" << port
                   << endl;
    return false;
  }
...
udr_socketpump::udr_socketpump(UDTSOCKET sock, int readhandle, int writehandle)
    : socket(sock), hread(readhandle), hwrite(writehandle) {}
...
void udr_socketpump::stop() { set_flag(do_stop); }
...
void *udr_socketpump::udt_read_func() {
  while (!(s_eof || s_err || h_werr || do_stop)) {
    ...
    goptions.dbg2() << "h_write() wrote " << bytes_read << " bytes"
                    << std::endl;
  }
...
bool udr_socketpump::h_read(size_t &bytes_read) {
  // input file may be non-blocking.  Let's poll it for read for 100ms so that
  // we can also quit if asked to
  struct pollfd fd;
  fd.fd = hread;
  fd.events = POLLIN;
  int r = poll(&fd, 1, 100);
...
// should be called with bytes_written as zero. will return without error in
// case of timeout. caller must call this repeatedly to empty the buffer.
...
udr_buffer::udr_buffer() : buf(0), bufsize(0), bufused(0) {}
...
char *udr_buffer::get() { return buf ? buf : sbuf; }

size_t udr_buffer::get_size() { return buf ? bufsize : sizeof(sbuf); }

size_t udr_buffer::get_used() { return bufused; }

bool udr_buffer::set_size(size_t s) {
  size_t oldsize = get_size();
  if (s <= oldsize)
    return true;
  goptions.dbg2() << "resize buf from " << oldsize << " to " << s << "bytes"
                  << std::endl;
  char *nbuf;
  if (buf) {
...
void udr_buffer::set_used(size_t s) {
  // growth policy.  If we used more than three quarters of the buf, double the
  // buffer
  if (s > ((get_size() * 3) >> 2))
    set_size(get_size() * 2);
  bufused = s;
